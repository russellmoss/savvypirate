...
const elements = {
    ...
    personSelectionSection: document.getElementById('personSelectionSection'),
 ...
 */
/**
 * Group searches by person (Source Connection)
 * Returns: Map of person name -> array of search indices
 */
function groupSearchesByPerson() {
    const personMap = new Map();
    
    state.searches.forEach((search, index) => {
        const personName = search.source || 'Unknown';
        if (!personMap.has(personName)) {
            personMap.set(personName, []);
        }
        personMap.get(personName).push(index);
    });
...
/**
 * Render person selection list
 */
function renderPersonList() {
    if (!elements.personList || !elements.personSelectionSection) {
        return;
    }
    
    if (state.searches.length === 0) {
        elements.personSelectionSection.style.display = 'none';
        return;
    }
    
    const personMap = groupSearchesByPerson();
    ...
    elements.personSelectionSection.style.display = 'block';
    
    // Render person checkboxes
    elements.personList.innerHTML = Array.from(personMap.entries())
        .sort(([a], [b]) => a.localeCompare(b)) // Sort alphabetically
        .map(([personName, searchIndices]) => {
            // Check if all searches for this person are selected
            const allSelected = searchIndices.every(idx => state.selectedSearches.has(idx));
            const someSelected = searchIndices.some(idx => state.selectedSearches.has(idx));
            ...
            if (allSelected) {
                ...
                checkboxState = 'checked'; // Will set indeterminate via JS
            }
            
            return `
                <div class="person-item" data-person="${escapeHtml(personName)}">
                    <label class="person-checkbox-label">
                        <input type="checkbox" 
                               class="person-checkbox" 
                               data-person="${escapeHtml(personName)}"
                               ${checkboxState}>
                        <span class="person-name">${escapeHtml(personName)}</span>
                        <span class="person-count">(${searchIndices.length} searches)</span>
                    </label>
                </div>
            `;
        }).join('');
    
    // Add change handlers for person checkboxes
    elements.personList.querySelectorAll('.person-checkbox').forEach(checkbox => {
        checkbox.addEventListener('change', (e) => {
            e.stopPropagation();
            handlePersonCheckboxChange(e);
        });
        
        // Set indeterminate state if some (but not all) searches are selected
        const personName = checkbox.dataset.person;
        const searchIndices = personMap.get(personName) || [];
        const allSelected = searchIndices.every(idx => state.selectedSearches.has(idx));
        const someSelected = searchIndices.some(idx => state.selectedSearches.has(idx));
...
/**
 * Handle person checkbox change - select/deselect all searches for that person
 */
function handlePersonCheckboxChange(event) {
    ...
    if (checkbox.checked) {
        // Select all searches for this person
        searchIndices.forEach(index => {
            state.selectedSearches.add(index);
        });
    } else {
        // Deselect all searches for this person
        searchIndices.forEach(index => {
            state.selectedSearches.delete(index);
        });
    }
    
    // Update search list checkboxes directly (don't re-render entire list)
    if (elements.searchList) {
        searchIndices.forEach(index => {
            const searchCheckbox = elements.searchList.querySelector(`.search-checkbox[data-index="${index}"]`);
            const searchItem = searchCheckbox?.closest('.search-item');
            
            if (searchCheckbox) {
                searchCheckbox.checked = checkbox.checked;
            }
    ...
    // Update person checkboxes state (for indeterminate handling)
...
function renderSearchList() {
    if (state.searches.length === 0) {
        ...
        if (elements.personSelectionSection) {
            elements.personSelectionSection.style.display = 'none';
        }
        return;
    }
    ...
    // Update person checkboxes to reflect current selection state
    updatePersonCheckboxes();
}

/**
 * Update person checkboxes to reflect current search selection state
 */
...
function handleSearchCheckboxChange(event) {
    ...
    // Update person checkboxes to reflect changes
    updatePersonCheckboxes();
...
async function handleLoadSearches() {
    ...
    try {
        ...
        // PHASE 8: Initialize mapping (but don't auto-select searches)
        // User will manually select what they want to run
        
        // Load saved mapping first, then render
        await loadSourceMapping();
        await renderSourceMapping(); // Phase 8: Show mapping interface
        
        // Reset auto-run UI to ensure it's not showing as running
        resetAutoRunUI();
...
function updateAutoRunButtonState() {
    ...
    // CRITICAL: Double-check actual running state from service worker if we think we're running
    // This prevents stale state from showing "Already Running" incorrectly
    if (state.isAutoRunning) {
        // Verify with service worker - if not actually running, reset state
        sendMessage('GET_AUTO_RUN_STATUS').then(response => {
            if (!response || !response.success || !response.isRunning) {
                console.log('[POPUP] Detected stale isAutoRunning state, resetting');
...
async function checkAutoRunStatus() {
    try {
        ...
        if (!response || !response.success) {
            console.log('[POPUP] No auto-run status available - resetting UI');
...
function resetAutoRunUI() {
    ...
    // Hide progress and log when not running
    if (elements.autoRunProgress) {
        elements.autoRunProgress.style.display = 'none';
    }
    if (elements.autoRunLog) {
        elements.autoRunLog.style.display = 'none';
    }
